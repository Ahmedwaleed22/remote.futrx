...
func TestStatusUsesInstalledContainerAndRedactsFailures(t *testing.T) {
	...
	response, _ := b.Handle(applications.Request{Method: "GET", Path: "status", Body: []byte(`{"container":"other"}`)})
...
func TestOperationsSerializeAndReportFailures(t *testing.T) {
	...
	result, _ := b.Handle(applications.Request{Method: "POST", Path: "sync"})
	if result.Status != http.StatusAccepted {
		t.Fatalf("status %d", result.Status)
	}
	result, _ = b.Handle(applications.Request{Method: "POST", Path: "restart"})
...
func TestRejectsInvalidTargetsAndUnknownActions(t *testing.T) {
	...
	for _, request := range []applications.Request{{Method: "POST", Path: "exec"}, {Method: "GET", Path: "sync"}} {
