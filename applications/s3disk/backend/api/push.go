...
const (
	...
	// to S3 does; application.json widens the transport timeout to cover this.
...
// takes file names alone: both directories involved belong to this application, so
...
func (b *backend) push(r applications.Request) applications.Response {
	...
	if mounted := b.command(ctx, "mountpoint", "-q", "--", b.target.mountpoint); mounted.Error != "" {
		return applications.Errorf(
			http.StatusConflict, "Nothing is mounted at %s", b.target.mountpoint)
	}

	destination := path.Join(b.target.mountpoint, b.target.uploadsDir)
...
// validUploadName refuses anything that is not a plain file name. The backend
