...
The URL of **any** file under the application's `ui/`, for applications, fonts, or data:
...
`false` when the application ships no `backend/`, or when none of its installs is
...
- **Subscriptions are dropped when your application is uninstalled**, so you do not
...
`console.info` tagged with the application id:
