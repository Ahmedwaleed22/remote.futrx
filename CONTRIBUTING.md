...
`ui/` is what makes an application a plugin rather than just a service: its
...
`backend/` is the third half, and the one that makes "everything is a plugin"
...
- **Go** 1.25+ (see `backend/go.mod`). Also required *at runtime* on any server
  that installs an application shipping a `backend/` directory, because plugin source
