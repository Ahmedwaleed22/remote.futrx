...
//
// Leaf module: it knows about the browser, never about what is being marked —
...
function api(): { highlights: HighlightRegistry; Highlight: HighlightConstructor } | null {
  try {
...
export function paintHighlight(name: string, ranges: readonly Range[]): void {
  const found = api();
  if (!found) return;
  found.highlights.set(name, new found.Highlight(...ranges));
}
