  ...
  if (record.error) {
    return (
      <div class="flex items-start gap-2.5 rounded-lg border border-accent-red/30 bg-accent-red/[0.08] px-3 py-2.5 text-[13px]">
        ...
        <div class="min-w-0">
          <div class="font-medium text-accent-red">
            Container endpoint unavailable
          </div>
  ...
  return (
    <>
      <Panel title="Overview">
        <Grid>
          ...
          <Field
            label="Processes"
            value={
              info.resources?.processes ? String(info.resources.processes) : "—"
            }
            mono
          />
          ...
          <Field
            ...
            value={info.bootAutostart ? "yes" : "no"}
            mono
          />
      ...
      {info.network && info.network.length > 0 && (
        <NetworkPanel ifaces={info.network} onRepair={onRepairNetwork} />
      )}
      ...
      {info.authBundles && info.authBundles.length > 0 && (
        <AuthBundlesPanel bundles={info.authBundles} />
      )}
    </>
  );
}
...
export function ContainerStateBadge({ state }: { state: string }) {
  ...
  return (
    <span
      class={`inline-flex items-center h-5 px-1.5 rounded text-[11px] font-medium ${tone}`}
    >
      {state.toLowerCase()}
    </span>
  );
  ...
  return (
    <Panel title="OS">
      <Grid>
        ...
        <Field
          label="CPU count"
          value={os.cpuCount ? String(os.cpuCount) : "—"}
          mono
        />
        <Field
          label="Uptime"
          value={os.uptimeSec ? formatDuration(os.uptimeSec) : "—"}
          mono
        />
      </Grid>
    </Panel>
  ...
  return (
    <Panel title="Resources">
      <Grid>
        <Field
          label="Memory used"
          value={`${formatBytes(res.memoryCurrentBytes)} / ${formatBytes(res.memoryTotalBytes)}`}
          mono
        />
        <Field
          label="Memory peak"
          value={formatBytes(res.memoryPeakBytes)}
          mono
        />
        <Field label="Swap" value={formatBytes(res.swapCurrentBytes)} mono />
        <Field
          label="Disk (rootfs)"
          value={formatBytes(res.diskUsageBytes)}
          mono
        />
        <Field
          label="CPU time"
          value={
            res.cpuUsageSeconds
              ? `${res.cpuUsageSeconds.toLocaleString()} s`
              : "—"
          }
          mono
        />
        <Field
          label="Processes"
          value={res.processes ? String(res.processes) : "—"}
          mono
        />
      </Grid>
    </Panel>
  ...
  return (
    <Panel title="Disk usage (inside container)">
      <div class="space-y-2">
        {disks.map((disk) => {
          const percent =
            disk.totalBytes && disk.usedBytes != null
          ...
          return (
            ...
            >
              <div class="flex items-center justify-between gap-2 min-w-0">
                <div class="font-mono text-[12.5px] text-ink-100 truncate">
                  {disk.mountPath}
                </div>
                <div class="text-[11px] text-ink-300 font-mono whitespace-nowrap">
                  {formatBytes(disk.usedBytes)} / {formatBytes(disk.totalBytes)}
                  {percent != null && (
                    <span class="ml-1.5 text-ink-400">({percent}%)</span>
                  )}
                </div>
              </div>
              {percent != null && (
...
function NetworkPanel({
  ...
  const noIPv4 = !ifaces.some((network) =>
    (network.addresses ?? []).some((address) =>
        ...
        {noIPv4 && (
          <div class="flex items-start gap-2.5 rounded-lg border border-accent-red/30 bg-accent-red/[0.08] px-3 py-2.5 text-[13px]">
            ...
            <div class="text-accent-red break-words">
              No IPv4 address — the container has no internet access. Use
              "Repair network" to re-run DHCP.
            </div>
          </div>
        )}
        {repairErr && (
        ...
        {ifaces.map((network) => (
          ...
          >
            <div class="flex items-center gap-2 min-w-0">
              <span class="font-mono text-[12.5px] text-ink-100">
                {network.name}
              </span>
              ...
              {network.macAddress && (
                <span class="ml-auto font-mono text-[11px] text-ink-400">
                  {network.macAddress}
                </span>
              )}
            </div>
            ...
            <div class="text-[11px] text-ink-400 font-mono">
              rx {formatBytes(network.bytesReceived)} · tx{" "}
  ...
  return (
    <Panel title="Claude provisioning">
      <Grid>
        <Field
          label="CLI installed"
          value={claude.installed ? "yes" : "no"}
          mono
        />
        <Field label="Version" value={claude.version || "—"} mono />
        <Field
          label="CLAUDE.md"
          value={claude.claudeMdInstalled ? "installed" : "missing"}
          mono
        />
        <Field
          ...
          mono
          tone={
            claude.claudeMdInstalled && !claude.claudeMdInSync
              ? "warn"
              : undefined
          }
        />
    ...
    <Panel title="Auth bundles">
      <div class="space-y-3">
        {bundles.map((bundle) => (
          ...
          >
            <div class="text-[12.5px] font-semibold text-ink-100">
              {bundle.name}
            </div>
...
function AuthFileRow({ file }: { file: AuthBundleFileStatus }) {
  ...
  return (
    <div class="rounded border border-white/[0.06] bg-black/20 px-2.5 py-1.5">
      <div class="font-mono text-[11.5px] text-ink-200 break-all">
        {file.containerPath}
      </div>
      <div class={`text-[11px] mt-0.5 ${tone}`}>{label}</div>
      <div class="text-[10.5px] font-mono text-ink-400 mt-0.5">
        host {file.hostExists ? formatUnixTime(file.hostMtime) : "—"} ·
        container{" "}
        {file.containerExists ? formatUnixTime(file.containerMtime) : "—"}
      </div>
    </div>
  );
}
