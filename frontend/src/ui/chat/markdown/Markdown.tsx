...
export function Markdown({
  ...
  return (
    <>
      {blocks.map((block, index) => renderBlock(block, `md-${index}`, context))}
    </>
  );
}
  ...
  switch (block.type) {
    ...
    case "code":
      return (
        <div
          key={key}
          class="relative my-3 rounded-lg border border-white/10 bg-[#101318] overflow-hidden"
        >
          {block.lang && (
    ...
    case "blockquote":
      return (
        <blockquote
          key={key}
          class="border-l-2 border-accent-blue/45 pl-3 my-2 text-ink-200 italic"
        >
          {block.children.map((child, index) =>
            renderBlock(child, `${key}-q-${index}`, context)
          )}
        </blockquote>
      );
    case "list":
    ...
    case "table":
      return (
        <div
          key={key}
          class="overflow-x-auto touch-scroll my-3 border border-white/10 rounded-lg"
        >
            ...
            <thead class="bg-white/[0.04]">
              <tr>
                {block.header.map((cell, index) => (
                  <th
                    key={index}
                    class="text-left px-3 py-1.5 font-semibold border-b border-white/10 text-ink-100"
                  >
              ...
              {block.rows.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  {row.map((cell, cellIndex) => (
                    <td
                      key={cellIndex}
                      class="px-3 py-1.5 border-b border-white/10"
                    >
                      {renderInline(
                        cell,
...
function renderHeading(
  ...
  if (level === 1)
    return (
      <h1 key={key} class="text-xl font-bold mt-3 mb-1.5">
        {renderInline(text, key, context)}
      </h1>
    );
  if (level === 2)
    return (
      <h2 key={key} class="text-lg font-bold mt-3 mb-1.5">
        {renderInline(text, key, context)}
      </h2>
    );
  return (
    <h3 key={key} class="text-base font-bold mt-2 mb-1">
      {renderInline(text, key, context)}
    </h3>
  );
}
...
function renderList(
  ...
  if (block.ordered) {
    return (
      <ol key={key} class={className} start={block.start}>
        {items}
      </ol>
    );
  }
  return (
    <ul key={key} class={className}>
      {items}
    </ul>
  );
}
