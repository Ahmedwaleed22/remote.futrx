  ...
  return (
    ...
    >
      ...
      >
        <div class="flex items-start gap-2">
          ...
          <div class="flex-1 min-w-0">
            <div
              class={`text-[13px] leading-snug truncate ${active ? "text-ink-50 font-medium" : "text-ink-100"}`}
            >
              {chat.title || "Untitled"}
            </div>
            <div class="mt-0.5 flex items-center gap-1.5 text-[11px] text-ink-400">
              <span
                class={`px-1 py-0.5 rounded bg-white/[0.06] text-[10px] leading-none whitespace-nowrap flex-none ${active ? "text-accent-blue" : ""}`}
              >
      ...
      <button
        ...
        aria-label={
          rawUnread
            ? `Mark ${chat.title || "chat"} read`
            : `Mark ${chat.title || "chat"} unread`
        }
