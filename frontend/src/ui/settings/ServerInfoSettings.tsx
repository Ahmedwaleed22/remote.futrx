    ...
    return (
      <div class="rounded-lg border border-accent-red/25 bg-accent-red/[0.06] p-4">
        <div class="flex items-start gap-2.5 text-[13px] text-accent-red">
          ...
          <span>
            Could not load server information{error ? `: ${error}` : "."}
          </span>
        </div>
        <button
          type="button"
    ...
    <div class="space-y-4">
      ...
      <InfoSection title="Host" Icon={Server}>
        <dl class="grid sm:grid-cols-2">
          ...
          <InfoField
            ...
            value={info.host.platform || info.host.os}
          />
          <InfoField label="Architecture" value={info.host.architecture} mono />
          <InfoField
            label="Kernel"
            value={info.host.kernel || "Unavailable"}
            mono
          />
          <InfoField
            label="App version"
            value={info.host.appVersion || "—"}
            mono
          />
          <InfoField label="Go runtime" value={info.host.goVersion} mono />
          <InfoField
            label="Host uptime"
            value={formatDuration(info.host.uptimeSec)}
          />
          <InfoField
            label="Service uptime"
            value={formatDuration(info.host.serviceUptimeSec)}
          />
          <InfoField
            label="Booted"
            value={formatDateTime(info.host.bootedAt)}
          />
      ...
      <div class="grid gap-4 lg:grid-cols-2">
        <InfoSection title="Processor" Icon={Cpu}>
          <dl class="grid grid-cols-2">
            ...
            <InfoField
              label="Logical cores"
              value={String(info.cpu.logicalCores)}
            />
            <InfoField
              label="Load · 1 min"
              value={formatNumber(info.cpu.loadAverage1)}
            />
            <InfoField
              label="Load · 5 min"
              value={formatNumber(info.cpu.loadAverage5)}
            />
            <InfoField
              label="Load · 15 min"
              value={formatNumber(info.cpu.loadAverage15)}
            />
        ...
        <InfoSection title="Memory" Icon={MemoryStick}>
          <dl class="grid grid-cols-2">
            ...
            <InfoField
              label="Free"
              value={formatBytes(info.memory.freeBytes)}
            />
            <InfoField
              label="Cached"
              value={formatBytes(info.memory.cachedBytes)}
            />
            <InfoField
              label="Buffers"
              value={formatBytes(info.memory.buffersBytes)}
            />
            <InfoField
              label="Swap used"
            ...
            <InfoField
              label="Swap free"
              value={formatBytes(info.memory.swapFreeBytes)}
            />
          </dl>
        </InfoSection>
      </div>
      ...
      <InfoSection title="Storage" Icon={HardDrive}>
        {info.storage.mounts.length === 0 ? (
          <EmptyInfo>
            Filesystem metrics are unavailable on this host.
          </EmptyInfo>
        ) : (
            ...
            {info.storage.mounts.map((mount) => (
              <div key={`${mount.device}:${mount.mountPath}`} class="p-4">
                <div class="flex items-start justify-between gap-4">
                  <div class="min-w-0">
                    <div
                      class="font-mono text-[13px] text-ink-100 truncate"
                  ...
                  <div class="text-right flex-none">
                    <div class="text-[13px] text-ink-100">
                      {formatPercent(mount.usagePercent)}
                    </div>
      ...
      <InfoSection title="Network" Icon={Network}>
        {info.network.interfaces.length === 0 ? (
          <EmptyInfo>
            Network interface metrics are unavailable on this host.
          </EmptyInfo>
        ) : (
          <div class="divide-y divide-white/[0.06]">
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex flex-wrap items-center gap-x-2">
                    <span class="font-mono text-[13px] text-ink-100">
      ...
      <InfoSection title="Remote process" Icon={Server}>
        <dl class="grid sm:grid-cols-3">
          ...
          <InfoField
            label="Goroutines"
            value={String(info.process.goroutines)}
          />
          <InfoField
            label="Open file handles"
            value={
              info.process.openFileHandles == null
                ? "Unavailable"
                : String(info.process.openFileHandles)
            }
          />
          <InfoField
            label="Allocated memory"
            value={formatBytes(info.process.allocatedBytes)}
          />
          <InfoField
            label="Heap in use"
            value={formatBytes(info.process.heapInUseBytes)}
          />
          <InfoField
            label="Go system memory"
            value={formatBytes(info.process.systemMemoryBytes)}
          />
        </dl>
      </InfoSection>
    </div>
...
function InfoField({
  ...
  return (
    <div class="min-w-0 px-4 py-3 border-b border-white/[0.06] sm:border-r">
      <dt class="text-[10.5px] uppercase tracking-wide text-ink-400">
        {label}
      </dt>
      <dd
